...
public class QuickPik implements IMainPresenter, EntryPoint {
	...
	private void bind() {
		...
		this.searchBar.getSearchTextBox().addKeyPressHandler(
				new KeyPressHandler() {
					@Override
		...
		this.resultsPane.addKeyUpHandler(new KeyUpHandler() {
			...
			public void onKeyUp(KeyUpEvent event) {
				...
				if (event.getNativeKeyCode() == KeyCodes.KEY_RIGHT) {
					...
					currentThumbIndex++;
					currentThumbIndex = Math.min(currentThumbIndex,
							thumbnails.size() - 1);
	...
	private Element getThumbnail(int index) {
		Element thumbnailsPanelElement = resultsPane.getThumbnailsPanel()
				.getElement();
		if (index < 0 || index >= thumbnailsPanelElement.getChildCount())
			return null;
	...
	private void newSearch() {
		...
		FilterLevel level = FilterLevel.valueOf(searchBar.getFilterValue()
				.toUpperCase());
	...
	private void updateNumberOfItems() {
		this.searchBar.getNumberOfResultsMessage().setText(
	...
	public void addPhotos(PhotosSearchResult result) {
		...
		searchBar.getSearchExpression().setText(
				RESULTS_FOR + result.getSearch());
	}
		...
		Scheduler.get().scheduleIncremental(new RepeatingCommand() {
			...
			public boolean execute() {
				if (photos.isEmpty()) {
					...
					final Thumbnail thumbnail = new Thumbnail(thumbnailImg,
							photoURL);
	...
	private void init() {
		...
		resultsPane.getThumbnailsPanel().getElement().getStyle()
				.setProperty("height", height, Unit.PX);
	...
	public void showMoreButton() {
		this.searchBar.getMoreResults().setVisible(true) ;
	}

	@Override
	public void hideMoreButton() {
		this.searchBar.getMoreResults().setVisible(false) ;
	}
}
